...
const char *Decompress::ARK_LIBRARY_PATH = _ARK_LIBRARY_PATH;
    ...
    std::function<void()> extractAll;
    {
        ...
        if ( converter == "auto" ) {
            ...
            extractAll =
                    [this]
                {
                    QDir pwd(QDir::current()); //현재 경로
                    int count = arkLib->GetFileItemCount(); //전체 압축 파일 내포 파일의 개수

                    // < -- 전체 압축 해제 크기 계산 -- >
                    std::function<void(const SArkFileItem *info)> reportTotalProgress;
                    if ( count > 0 && arkLib->GetFileItem(0)->uncompressedSize == (INT64)ARK_FILESIZE_UNKNOWN ) {
                        INT64 totalArchiveSize = 0; // 전체 압축 크기
                        for (int i = 0; i < count; i++) {
                            totalArchiveSize += arkLib->GetFileItem(i)->compressedSize;
                        }
                        reportTotalProgress =
                                [totalArchiveSize](
                                    const SArkFileItem *info
                                    )
                            {
                                if( info->compressedSize == 0 || totalArchiveSize == 0 ) {
                                    Report::getInstance()->setTotalPercent(0);
                                }
                                else {
                                    Report::getInstance()->setTotalPercent( info->compressedSize / totalArchiveSize );
                                }
                            };
                    }
                    else {
                        INT64 totalArchiveSize = 0; // 전체 압축 크기
                        for (int i = 0; i < count; i++) {
                            totalArchiveSize += arkLib->GetFileItem(i)->uncompressedSize;
                        }
                        reportTotalProgress =
                            [totalArchiveSize](
                                const SArkFileItem *info
                            ...
                            {
                                if ( info->uncompressedSize == 0 || totalArchiveSize == 0 ) {
                                    Report::getInstance()->setTotalPercent(0);
                                }
                                else {
                                    Report::getInstance()->setTotalPercent( info->uncompressedSize / totalArchiveSize );
                                }
                            };
                    }
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        if ( info->IsFolder() ) {
                            ...
                            arkLib->ExtractOneAs(i, uniquePath.filePath());
                            if ( duplicated ) {
                                Report::getInstance()->setNewName(uniquePath.fileName());
                            }
                        }
                        reportTotalProgress(info);
                    }
                };
        }
    }
        ...
        getSaveDirPath_ =
                    ...
                    ) -> QString
                {
                    ...
                    try {
                        ...
                        Report::getInstance()->setMakeFailSeperatedFolder(currentFilePath_, parentDir);
                        exitcode = 14;
                        return parentDir;
                    }
...
void Decompress::run()
{
    ...
    for(int i = 0; i < totalFileCount_; i++){
        ...
        //작업할 압축 파일을 보여줍니다.
        Report::getInstance()->setStartFile(i, currentFilePath_);

        //압축 파일 열기
        if( ! arkLib->Open(file) ){
            //압축 파일을 열수 없다면
            Report::getInstance()->setOpenFileError(currentFilePath_);
            exitcode = 15;
            continue;
        }

        //설정된 암호를 압축 해제할 파일에 적용
        arkLib->SetPassword(password_);
