...
public:
    ...
    CArkEvent* getArkEvent();
    int getExitcode();
    bool isWorkEnd();

private:
    ArkLib *arkLib;                                                    ///< 압축 해제 처리 객체(전방 선언됨)
    CArkEvent *arkEvent;                                               ///< 압축 해제 이벤트 처리(전방 선언됨)
    QVector<QFile*> *files;                                            ///< 파일 목록
    ...
    const static char *ARK_LIBRARY_PATH;                               ///< ark 라이브러리 경로
